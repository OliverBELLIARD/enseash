...
int main(int argc, char *argv[]) {
    ...
    // Fork a background process for check_background_processes
    ...
    if (background_pid == -1) {
        ...
        // This is the child process (background process)
        while (1) {
        ...
        while (1) {
            ...
            //
            if (eval(buf)) {
                // The "exit" command has been used
                break;
            }
        }

        // We terminate the background processes checker
...
ssize_t read_command(char *buf) {
    ...
    ssize_t bytesRead = read(STDIN_FILENO, buf, BUFSIZE - 1);
    if (bytesRead > 0) {
        // We reset the unused values of the buffer
        buf[bytesRead - 1] = '\0'; // Null-terminate the string
    }
    return bytesRead;
}
...
int eval(char *command) {
    ...
    //
    // ERROR VERBOSE REDIRECTION
    //
    // Suppress perror() messages for redirections
    ...
    //
    // ERROR VERBOSE REDIRECTION TERMINATION
    //
